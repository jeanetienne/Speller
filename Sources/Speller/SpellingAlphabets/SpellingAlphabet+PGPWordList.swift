...
public extension SpellingAlphabet {
    ...
    static let pgpWordList = PGPWordList(
        uniqueIdentifier: "speller.alphabet.pgp_word_list",
        associatedLanguageCode: "en",
        associatedRegionCode: nil,
        recommendedJoinerWord: "",
        mainContent: [
...
public class PGPWordList: SpellingAlphabet {

    public override func spell(_ phrase: String, withNumbers: Bool) -> [SpelledCharacter] {
    ...
    private func spell(character: String, atIndex index: Int) -> SpelledCharacter {
        if let codeWordCollection = codeWordCollection(forCharacter: character) {
            let singleCodeWordCollection = CodeWordCollection(codeWord: codeWordCollection.codeWords[index % 2])
            return SpelledCharacter(character: character, position: index, spellingResult: .match(singleCodeWordCollection))
        } else {
            return SpelledCharacter(character: character, position: index, spellingResult: .unknown)
        }
    }

    private func codeWordCollection(forCharacter character: String) -> CodeWordCollection? {
